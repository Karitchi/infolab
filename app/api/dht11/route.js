...
export async function POST(request) {
  try {
    ...
    if (temperature && humidity && !isNaN(Number(temperature)) && !isNaN(Number(humidity))) {
      // Mettre à jour les données si elles sont valides
      sensorData = { temperature: Number(temperature), humidity: Number(humidity) };
      ...
      return NextResponse.json({ message: "Données invalides ignorées" }, { status: 400 });
    }
  } catch (error) {
